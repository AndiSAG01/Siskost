...
@section('content')
    <div class="section">
        <div class="container py-32">
            <h1
                ...
                FORM SEWA KOST TAHUNAN
            </h1>
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('sewaPertahunStore') }}" method="POST">
                        @csrf
                        <div class="row">
                                        ...
                                        <figcaption class="absolute bottom-8 left-2/4 flex w-[calc(100%-4rem)] -translate-x-2/4 justify-between rounded-xl border border-white bg-white/75 py-4 px-6 shadow-lg shadow-black/5 saturate-200 backdrop-blur-sm">
                                            <div>
                                                ...
                                                <p class="block mt-2 font-sans text-base antialiased font-normal leading-relaxed text-gray-700">
                                                    Rp.{{ number_format($kost->price_years, 2) }}/Bulan
                                                </p>
                                            </div>
                        ...
                        <div class="card">
                            <div class="card-body">
                                <div class="card-text mb-2">
                                    <h3 class="uppercase font-semibold">Jumlah Sewa : <span id="rent_duration">/Tahun</span></h3>
                                </div>
                                <hr>
    ...
    <script>
        function calculateRent() {
            ...
            const annualRent = {{ $kost->price_years }}; // Asumsi nilai ini tersedia dari backend
    
            if (startDate && endDate && !isNaN(annualRent)) {
                const diffInYears = endDate.getFullYear() - startDate.getFullYear() + (endDate.getMonth() - startDate.getMonth()) / 12;
                const totalRentCost = diffInYears * annualRent;
    
                document.getElementById('rent_duration').innerText = `${diffInYears.toFixed()} Tahun`;
